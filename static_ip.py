...
DHCPCD_CONF = "/home/fw/dns-firewall/dhcpcd.conf"  # TODO: Changed for tests!
DHCPCD_CONF_ORIGINAL = DHCPCD_CONF + ".original"
...
def configure(use_info=False, self_as_resolver=False) -> None:
    ...
    try:
        ...
        shutil.move(DHCPCD_CONF_ORIGINAL, DHCPCD_CONF)
...
def revert() -> None:
    if not is_configured():
        return
    if os.path.isfile(DHCPCD_CONF_ORIGINAL):
        shutil.copy2(DHCPCD_CONF_ORIGINAL, DHCPCD_CONF)
        try:
            _reboot_network()
        except bash.CallError as error:
            ...
            os.remove(DHCPCD_CONF_ORIGINAL)
            os.remove(INFO_FILE)
    else:
